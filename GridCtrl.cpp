...
////fn by milind to force move cell ptr before save
...
//	SetFocusCell(-1,-1);
		//HWND hmm=GetWindow(0);

	///CWnd cc::GetParent();
//	::SendMessage(this,WM_KEYDOWN,VK_DOWN,0);
...
void CGridCtrl::OnEditCell(int nRow, int nCol, UINT nChar)
{
    ...
    DWORD dwStyle = ES_LEFT | ES_UPPERCASE; ///???? changed by milind for default ucase
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
	...
	if (strCurrent != str)
    {
      ...
      if(nCol>0)
	  {
          ...
          str.Empty();
		 for(i=0;i<Batch.GetSize();i++)
		 {   part=Batch.GetAt(i);
		     part.TrimLeft();part.TrimRight();
		     if(part.GetLength()==5) 
			 { part.Insert(5,')'); part.Insert(3,'('); }
			 
			 str+=part;
			 if(i!=Batch.GetSize()-1) str+=',';
		 }
		...
		/// milind - color numeric entry
		if(str[0]>='0' && str[0]<='9') SetItemBkColour(nRow,nCol,RGB(200,255,200));
	}
}
...
int iBlocks = (str.GetLength())/5, iLeftBrace = 3 , iRightBrace = 6  ;



//m_str.Insert(3,'(');m_str.Insert(m_str.GetLength(),')');
	
	
		for(int p = 1; p <= iBlocks; p++)
		{
			str.Insert(iLeftBrace,'(');
			str.Insert(iRightBrace,')');

			iLeftBrace = iLeftBrace + 8;
			iRightBrace = iRightBrace + 8;
...
void CGridCtrl::SetFocusPublic(int nRow, int nCol)
{ // int rr=m_idCurrentCell.row;
   //int cc=m_idCurrentCell.col;
   //int nState = GetItemState(rr,cc);
